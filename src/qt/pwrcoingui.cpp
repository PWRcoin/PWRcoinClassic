 ...
 * The pwrcoin developers 2011-2012
 */
#include "pwrcoingui.h"
...
pwrcoinGUI::pwrcoinGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
...
{
    resize(850, 550);
    setWindowTitle(tr("pwrcoin") + " - " + tr("Wallet"));
    // Prevent resizing.
    //setFixedSize(size());
    // Remove "hand" cursor from status bar.
    this->statusBar()->setSizeGripEnabled(false);

    this->setStyleSheet(".pwrcoinGUI \
                        ...
                        ");
#ifndef Q_OS_MAC
    qApp->setWindowIcon(QIcon(":icons/pwrcoin"));
    setWindowIcon(QIcon(":icons/pwrcoin"));
#else
...
void pwrcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a pwrcoin address"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for pwrcoin"));
    optionsAction->setMenuRole(QAction::PreferencesRole);
    toggleHideAction = new QAction(QIcon(":/icons/pwrcoin"), tr("&Show / Hide"), this);
...
void pwrcoinGUI::createToolBars()
{
...
void pwrcoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifndef Q_OS_MAC
            qApp->setWindowIcon(QIcon(":icons/pwrcoin_testnet"));
            setWindowIcon(QIcon(":icons/pwrcoin_testnet"));
#else
            MacDockIconHandler::instance()->setIcon(QIcon(":icons/pwrcoin_testnet"));
#endif
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("pwrcoin client") + QString(" ") + tr("[testnet]"));
...
void pwrcoinGUI::setWalletModel(WalletModel *walletModel)
{
...
void pwrcoinGUI::createTrayIcon()
{
    QMenu *trayIconMenu;
#ifndef Q_OS_MAC
    ...
    trayIcon->setToolTip(tr("pwrcoin client"));
...
#ifndef Q_OS_MAC
void pwrcoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void pwrcoinGUI::optionsClicked()
{
...
void pwrcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to pwrcoin network", "", count));
}

void pwrcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
...
void pwrcoinGUI::error(const QString &title, const QString &message, bool modal)
{
    // Report errors from network/worker thread
    if(modal)
    {
...
void pwrcoinGUI::changeEvent(QEvent *e)
{
...
void pwrcoinGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void pwrcoinGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    QString strMessage =
        tr("This transaction is over the size limit.  You can still send it for a fee of %1, "
          "which goes to the nodes that process your transaction and helps to support the network.  "
          "Do you want to pay the fee?").arg(
                pwrcoinUnits::formatWithUnit(pwrcoinUnits::PWR, nFeeRequired));
...
void pwrcoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
        ...
        notificator->notify(Notificator::Information,
                            (amount)<0 ? tr("Sent transaction") :
                              ...
                              tr("Date: %1\n"
                                 ...
                                 "Address: %4\n")
                              .arg(date)
                              .arg(pwrcoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                              .arg(type)
                              .arg(address), icon);
    }
}

void pwrcoinGUI::gotoOverviewPage()
{
...
void pwrcoinGUI::gotoHistoryPage()
{
...
void pwrcoinGUI::gotoAddressBookPage()
{
...
void pwrcoinGUI::gotoReceiveCoinsPage()
{
...
void pwrcoinGUI::gotoSendCoinsPage()
{
...
void pwrcoinGUI::gotoSignMessageTab(QString addr)
{
...
void pwrcoinGUI::gotoVerifyMessageTab(QString addr)
{
...
void pwrcoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void pwrcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid pwrcoin address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void pwrcoinGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void pwrcoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void pwrcoinGUI::encryptWallet(bool status)
{
    if(!walletModel)
        return;
...
void pwrcoinGUI::changePassphrase()
{
...
void pwrcoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void pwrcoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void pwrcoinGUI::updateStakingIcon()
{
