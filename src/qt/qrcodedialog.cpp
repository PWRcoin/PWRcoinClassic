...
{
    QString ret = QString("powercoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PWR unit input in lnReqAmount, we generate the URI with PWR as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(powercoinUnits::format(powercoinUnits::PWR, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
