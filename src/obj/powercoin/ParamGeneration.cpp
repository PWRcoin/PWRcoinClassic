...
///
/// \brief      Parameter manipulation routines for the Powercoin cryptographic
///             components.
///
...
/// \brief Fill in a set of Powercoin parameters from a modulus "N".
...
///
/// \throws         PowercoinException if the process fails
///
...
/// be known to any party, or the security of Powercoin is
...
CalculateParams(Params &params, Bignum N, string aux, uint32_t securityLevel)
{
	...
	if (NLen < 1023) {
		throw PowercoinException("Modulus must be at least 1023 bits");
	}

	// Verify that "securityLevel" is  at least 80 bits (minimum).
	if (securityLevel < 80) {
		throw PowercoinException("Security level must be at least 80 bits.");
	}
	...
	*pLen = *qLen = 0;
	...
	if (securityLevel < 80) {
		...
		*pLen = 3072;
		...
		throw PowercoinException("Security level not supported.");
	}

	if (*pLen > maxPLen) {
		throw PowercoinException("Modulus size is too small for this security level.");
	}
}
...
deriveIntegerGroupParams(uint256 seed, uint32_t pLen, uint32_t qLen)
{
	...
	if ((uint32_t)(result.modulus.bitSize()) < pLen ||          // modulus is pLen bits long
		...
		throw PowercoinException("Group parameters are not valid");
	}
...
deriveIntegerGroupFromOrder(Bignum &groupOrder)
{
	...
	throw PowercoinException("Too many attempts to generate Schnorr group.");
}
                              ...
                              uint256 *resultPseed, uint256 *resultQseed)
{
	...
	if (qLen > (sizeof(seed)) * 8) {
		...
		// throw PowercoinException("Seed is too short to support the required security level.");
	}

#ifdef Powercoin_DEBUG
	...
	*resultGroupOrder = generateRandomPrime(qLen, seed, &qseed, &qgen_counter);
	...
	// Throw an exception.
	throw PowercoinException("Unable to generate a prime modulus for the group");
}
...
/// \throws                             A PowercoinException if error.
///
...
calculateGroupGenerator(uint256 seed, uint256 pSeed, uint256 qSeed, Bignum modulus, Bignum groupOrder, uint32_t index)
{
	...
	if (index > 255) {
		throw PowercoinException("Invalid index for group generation");
	}
	...
	throw PowercoinException("Unable to find a generator, too many attempts");
}
                    ...
                    uint32_t *prime_gen_counter)
{
	...
	if (primeBitLen < 2) {
		throw PowercoinException("Prime length is too short");
	}
	...
	if (primeBitLen < 33) {
		...
		throw PowercoinException("Unable to find prime in Shawe-Taylor algorithm");
	...
	throw PowercoinException("Unable to generate random prime (too many tests)");
}
