...
* @file       Tests.cpp
*
* @brief      Test routines for Powercoin.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", Powercoin_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),Powercoin_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	printf("Powercoin v%s self-test routine\n", Powercoin_VERSION_STRING);
	...
	// Run through all of the Powercoin tests
