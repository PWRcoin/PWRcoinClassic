...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // pwrcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // pwrcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// pwrcoin: find block wanted by given orphan block
...
static const int64_t nTargetTimespan = 10 * 60;  // 10 min
//
...
// pwrcoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // pwrcoin: target change every block
    // pwrcoin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    // fMiner is true when called from the internal pwrcoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // pwrcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // pwrcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // pwrcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
...
// pwrcoin: total coin age spent in transaction, in the unit of coin-days.
...
// pwrcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // pwrcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // pwrcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // pwrcoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // pwrcoin: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // PWRcoin: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // pwrcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // pwrcoin: getblocks may not obtain the ancestor block rejected
    ...
    // pwrcoin: if responsible for sync-checkpoint send it
...
// pwrcoin: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "pwrcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    ZCParams = new libPWRcoin::Params(bnTrustedModulus);
#endif
	...
	uint256 actualGenesisBlock = !fTestNet ? hashGenesisBlock : hashGenesisBlockTestNet;
        ...
        //// debug print
         ...
         if(!fTestNet)
        {
            assert(block.hashMerkleRoot == uint256("0x8777033e3ddd2c2ba4762489c58e3c389b26b838a0f5df92a026b12d735fb228"));
        }
        else
        {
            assert(block.hashMerkleRoot == uint256("0x0bae0383a1ae2543e1ade5822717da18e9c1521caedf56a32805a0aa9091c58b"));
        }                                                //Testnet Hash
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // pwrcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // pwrcoin: tell downloading node about the latest block if it's
