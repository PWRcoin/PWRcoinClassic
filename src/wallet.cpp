...
// pwrcoin: total coins staked (non-spendable until maturity)
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    // change transaction isn't always pay-to-pwrcoin-address
...
// PWRcoin: get current stake weight
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CpwrcoinAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CpwrcoinAddress(address).ToString());
}
...
// pwrcoin: check 'spent' consistency between wallet and txindex
// pwrcoin: fix wallet spent state according to txindex
...
// pwrcoin: disable transaction (only for coinstake)
