...
* @file       Tests.cpp
*
* @brief      Test routines for PWRcoin.
*
...
Test_GenerateGroupParams()
{
	...
	for (count = 0; count < 1; count++) {
		...
		try {
			group = deriveIntegerGroupParams(calculateSeed(GetTestModulus(), "test", PWRcoin_DEFAULT_SECURITYLEVEL, "TEST GROUP"), pLen, qLen);
...
Test_ParamGen()
{
	...
	try {
		...
		Params testParams(GetTestModulus(),PWRcoin_DEFAULT_SECURITYLEVEL);
...
Test_RunAllTests()
{
	printf("PWRcoin v%s self-test routine\n", PWRcoin_VERSION_STRING);
	...
	// Run through all of the PWRcoin tests
